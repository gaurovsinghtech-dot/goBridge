...
Route::middleware('throttle:webhooks')->group(function () {
    ...
    // ─── Twilio Telephony & AI Voice Webhooks ──────────────────────────────────
    Route::match(['get', 'post'], 'webhooks/twilio/voice', [TwilioWebhookController::class, 'handleVoice'])
        ->name('webhooks.twilio.voice');
    Route::match(['get', 'post'], 'webhooks/twilio/gather', [TwilioWebhookController::class, 'handleGather'])
        ->name('webhooks.twilio.gather');
    Route::match(['get', 'post'], 'webhooks/twilio/sms', [TwilioWebhookController::class, 'handleSms'])
        ->name('webhooks.twilio.sms');
    Route::match(['get', 'post'], 'webhooks/twilio/status', [TwilioWebhookController::class, 'handleStatus'])
        ->name('webhooks.twilio.status');

    Route::match(['get', 'post'], 'api/v1/webhooks/twilio/voice', [TwilioWebhookController::class, 'handleVoice'])
        ->name('api.webhooks.twilio.voice');
    Route::match(['get', 'post'], 'api/v1/webhooks/twilio/gather', [TwilioWebhookController::class, 'handleGather'])
        ->name('api.webhooks.twilio.gather');
    Route::match(['get', 'post'], 'api/v1/webhooks/twilio/sms', [TwilioWebhookController::class, 'handleSms'])
        ->name('api.webhooks.twilio.sms');
    Route::match(['get', 'post'], 'api/v1/webhooks/twilio/status', [TwilioWebhookController::class, 'handleStatus'])
        ->name('api.webhooks.twilio.status');
