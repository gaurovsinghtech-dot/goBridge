    ...
    Route::get('/health', function () {
        try {
            ...
            return response()->json([
                'status' => 'healthy',
                'app' => config('app.name', 'Growbridge Connect'),
                'timestamp' => now()->toIso8601String(),
                'storage' => $storageOk ? 'writable' : 'read-only',
            ]);
        } catch (\Throwable $e) {
            return response()->json(['status' => 'degraded', 'message' => 'Service degraded'], 503);
        }
    })->name('health');
});

// Android APK Direct Download & Scannable QR Code
Route::get('/download/android-apk', [\App\Http\Controllers\Client\AppDownloadController::class, 'downloadApk'])->name('download.android-apk');
Route::get('/download/android-apk/qr', [\App\Http\Controllers\Client\AppDownloadController::class, 'qrCode'])->name('download.android-apk.qr');

// Secure Object Storage Preview & Download
Route::get('/storage/preview/{uuid}', [\App\Http\Controllers\Client\StorageFileController::class, 'preview'])->name('client.storage.preview');
Route::get('/storage/download/{uuid}', [\App\Http\Controllers\Client\StorageFileController::class, 'download'])->middleware(['auth'])->name('client.storage.download');
