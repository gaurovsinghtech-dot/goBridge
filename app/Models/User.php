...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        return $this->currentWorkspace ?? $this->ownedWorkspaces()->first();
    }
    ...
    public function productTours(): HasMany
    {
        return $this->hasMany(UserProductTour::class);
    }

    public function productTour(string $tourKey = 'dashboard_tour'): ?UserProductTour
    {
        return $this->productTours()->where('tour_key', $tourKey)->first();
    }
    ...
    {
        try {
            if (request()->hasSession() && request()->session()->has('current_workspace_id')) {
                return (int) request()->session()->get('current_workspace_id');
            }
        } catch (\Throwable) {
            // CLI or stateless context
        }

        return $this->workspace_id ? (int) $this->workspace_id : null;
    }

    public function hasEntitlement(string $feature): bool
    {
        $workspaceId = $this->current_workspace_id ?? $this->workspace_id;
        $workspace = $workspaceId ? Workspace::find($workspaceId) : ($this->workspace ?? $this->ownedWorkspaces()->first());

        return \App\Services\Billing\EntitlementService::can($workspace, $feature);
    }
    ...
    public function entitlements(): array
    {
        ...
        return \App\Services\Billing\EntitlementService::getEntitlements($workspace ?? 0);
    }
}
