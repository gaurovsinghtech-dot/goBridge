...
class HandleInertiaRequests extends Middleware
{
    ...
    private function brandingShare(): array
    {
        try {
            ...
            return [
                ...
                'primary_color' => SystemSetting::get('primary_color') ?: config('saas.branding.primary_color', '#10b981'),
                'secondary_color' => SystemSetting::get('secondary_color') ?: config('saas.branding.secondary_color', '#064e3b'),
                'font_family' => SystemSetting::get('font_family') ?: config('saas.branding.font_family', 'space-grotesk'),
                'logo_url' => $logoPath ? $this->assetUrl($logoPath, SystemSetting::get('app_logo_disk', 'public')) : '/images/brand/logo-full.png',
                'logo_icon_url' => '/images/brand/logo-icon.png',
                'favicon_url' => $faviconPath ? $this->assetUrl($faviconPath, SystemSetting::get('app_favicon_disk', 'public')) : '/images/brand/logo-icon.png',
            ];
        } catch (\Throwable) {
            return [
                ...
                'primary_color' => config('saas.branding.primary_color', '#10b981'),
                'secondary_color' => config('saas.branding.secondary_color', '#064e3b'),
                'font_family' => config('saas.branding.font_family', 'space-grotesk'),
                'logo_url' => '/images/brand/logo-full.png',
                'logo_icon_url' => '/images/brand/logo-icon.png',
                'favicon_url' => '/images/brand/logo-icon.png',
            ];
        }
    }
    ...
    private function appShare(Request $request): array
    {
        ...
        $i18nLocales = Locale::forSwitcherCached();
        ...
        $currencies = Currency::enabledCurrenciesList();
        ...
        $entitlements = ($workspace ?? null)
            ? \App\Services\Billing\EntitlementService::getEntitlements($workspace)
            : ($user ? \App\Services\Billing\EntitlementService::getEntitlements($user->current_workspace_id ?? $user->workspace_id ?? 0) : \App\Services\Billing\EntitlementService::DEFAULT_PLAN_ENTITLEMENTS['starter']);
        ...
        return [
            ...
            'i18n' => [
                ...
                'isRtl' => $isRtl,
                'locales' => $i18nLocales,
                'translations' => fn () => app(I18nFileService::class)->getFlatDictionary($locale),
            ],
            ...
            'dashboard_tour' => $this->dashboardTourShare($user, $isAdminRoute),
        ];
    }

    private function dashboardTourShare(?User $user, bool $isAdminRoute): ?array
    {
        ...
        try {
            $tour = \App\Models\UserProductTour::where('user_id', $user->id)
                ->where('tour_key', 'dashboard_tour')
                ->first();

            return [
                'should_show' => ! $tour || (! $tour->isCompleted() && ! $tour->isSkipped()),
                'current_step' => (int) ($tour?->current_step ?? 0),
                'is_completed' => (bool) $tour?->isCompleted(),
                'is_skipped' => (bool) $tour?->isSkipped(),
            ];
        } catch (\Throwable) {
            return null;
        }
