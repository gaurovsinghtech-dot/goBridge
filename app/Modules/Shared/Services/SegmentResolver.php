...
class SegmentResolver
{
    private const ALLOWED_FIELDS = [
        ...
        'country', 'language', 'source', 'lead_score', 'lead_temperature',
        'pipeline_stage', 'status',
        'opt_in_whatsapp', 'opt_in_sms', 'opt_in_email', 'opt_out_all',
        'created_at', 'last_seen_at',
    ];

    private const OPERATORS = [
        '=', '!=', 'like', 'not_like', '<', '>', '<=', '>=',
        'is_null', 'is_not_null', 'tag_contains', 'tag_not_contains',
    ...
    /** Preview contact count for given raw rules without saving. */
    public function previewCount(int $workspaceId, array $rules): int
    {
    ...
    public function applyRules(Builder $query, array $rules): void
    {
        ...
        foreach ($rules['conditions'] ?? [] as $condition) {
            ...
            if ($operator === 'tag_contains' && ! empty($value)) {
                $query->whereHas('tags', fn ($q) => $q->where('name', 'like', '%' . $value . '%'));
                continue;
            }

            if ($operator === 'tag_not_contains' && ! empty($value)) {
                $query->whereDoesntHave('tags', fn ($q) => $q->where('name', 'like', '%' . $value . '%'));
                continue;
            }

            if ($operator === 'older_than_days' && is_numeric($value)) {
                $date = now()->subDays((int) $value);
                $query->{$method}($field, '<=', $date);
                continue;
            }

            if ($operator === 'within_days' && is_numeric($value)) {
                $date = now()->subDays((int) $value);
                $query->{$method}($field, '>=', $date);
                continue;
            }
