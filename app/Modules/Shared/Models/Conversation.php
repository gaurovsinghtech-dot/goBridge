...
class Conversation extends Model
{
    ...
    protected $fillable = [
        'uuid', 'workspace_id', 'channel_account_id', 'contact_id', 'channel',
        'external_thread_id', 'status', 'assigned_user_id', 'assigned_ai_agent_id',
        'ai_agent_id', 'ai_mode', 'handoff_reason', 'human_takeover_at', 'ai_last_response_at',
        'confidence_score', 'assigned_to', 'handover_at', 'last_message_at', 'unread_count',
    ...
    {
        return ($this->ai_mode === 'auto' || $this->assigned_to === 'bot' || $this->assigned_to === 'ai')
            && $this->assigned_to !== 'human'
            && $this->ai_mode !== 'paused';
    }

    public function isAiSuggested(): bool
    {
        return $this->ai_mode === 'suggested';
    }

    public function isHumanActive(): bool
    {
        return $this->ai_mode === 'human' || $this->assigned_to === 'human';
    }

    public function isPaused(): bool
    {
        return $this->ai_mode === 'paused';
    }
    ...
    public function isWhatsappWindowOpen(): bool
    {
        ...
        $inboundTime = \Illuminate\Support\Carbon::parse($latestInbound);

        return $inboundTime->diffInRealHours(now()) < 24;
    }
}
