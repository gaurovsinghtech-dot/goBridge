...
class SendCampaignMessageJob implements ShouldQueue
{
    ...
    /**
     * Determine if an error is permanent (do not retry) vs transient (retry).
     */
    private function isPermanentFailure(string $errorMessage): bool
    {
        ...
        $permanentPatterns = [
            ...
            '131051', // Meta: unsupported message type
            '131047', // Meta: re-engagement window expired
            '132001', // Meta: template does not exist
            '132000', // Meta: template parameter count mismatch
            'invalid_recipient',
            'syntax error',
            'unsupported channel',
        ];
     ...
     */
    private function sendInstagram(Campaign $campaign, Contact $contact, CampaignPersonalizer $personalizer): array
    {
        $rawBody = $campaign->payload_json['body'] ?? '';
        $body = $personalizer->renderString($rawBody, $contact);

        if ($body === '') {
            throw new \RuntimeException('Instagram campaign message body cannot be empty.');
        }

        $messageId = 'IG-'.Str::random(16);
     ...
     * @return array{id: string, body: string, type: string, payload: array<string, mixed>}
     */
    private function sendMessenger(Campaign $campaign, Contact $contact, CampaignPersonalizer $personalizer): array
    {
        $rawBody = $campaign->payload_json['body'] ?? '';
        $body = $personalizer->renderString($rawBody, $contact);

        if ($body === '') {
            throw new \RuntimeException('Messenger campaign message body cannot be empty.');
        }

        $messageId = 'FB-'.Str::random(16);
