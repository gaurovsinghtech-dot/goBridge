...
class AutomationEngine
{
    ...
    public function triggerForContact(Automation $automation, int $contactId, array $context = [], ?string $idempotencyKey = null): ?AutomationRun
    {
        ...
        // Verify feature entitlement when workspace subscription exists
        if (\App\Models\Subscription::where('workspace_id', $workspaceId)->exists()) {
            if (! \App\Services\Billing\EntitlementService::can($workspaceId, 'automations')
                && ! \App\Services\Billing\EntitlementService::can($workspaceId, 'advanced_automation')) {
                Log::warning("Automation {$automation->id} skipped: workspace {$workspaceId} has no automations entitlement.");
                return null;
            }

            // Verify usage quota
            $usageService = app(\App\Services\Billing\UsageService::class);
            if (! $usageService->canConsume($workspaceId, 'automation_runs', 1)) {
                Log::warning("Automation {$automation->id} skipped: workspace {$workspaceId} exhausted automation runs quota.");
                return null;
            }
        }

        $key = $idempotencyKey ?: ($context['event_id'] ?? ($context['idempotency_key'] ?? null));
        if (! empty($key)) {
            $exists = AutomationRun::where('automation_id', $automation->id)
                ->where('idempotency_key', $key)
                ->exists();
            if ($exists) {
                Log::info("Automation {$automation->id} skipped duplicate event with idempotency key: {$key}");
                return null;
            }
        }
        ...
        $run = AutomationRun::create([
            ...
            'trigger_event' => $automation->trigger_type ?: 'event',
            'idempotency_key' => $key,
            'status' => 'pending',
            'step_count' => 0,
            'max_steps' => 100,
    ...
    {
        $run->refresh();
        if (in_array($run->status, ['paused', 'cancelled', 'completed', 'failed'], true)) {
            return;
        }
        ...
        $visited = [];
        $stepIndex = (int) ($run->step_count ?? 0);
        $maxSteps = (int) ($run->max_steps ?: 100);
        $maxDuration = (int) ($run->max_duration_seconds ?: 300);
        $runStartedAt = $run->started_at ?: now();

        while ($currentId && $stepIndex < $maxSteps) {
            // Check if run was paused or cancelled
            $freshRun = $run->fresh();
            if (in_array($freshRun->status, ['paused', 'cancelled'], true)) {
                return;
            }

            // Duration watchdog check
            if (now()->diffInSeconds($runStartedAt) > $maxDuration) {
                $run->update([
                    'status' => 'failed',
                    'error' => "Execution exceeded maximum allowed duration of {$maxDuration}s. Terminated to prevent infinite loop.",
                    'completed_at' => now(),
                ]);
                return;
            }
            ...
            $cycleCount = count(array_keys($visited, $currentId));
            if ($cycleCount >= 5) {
                $run->update([
                    'status' => 'failed',
                    'error' => "Infinite loop detected at node {$currentId}. Cycle count exceeded threshold.",
                    'completed_at' => now(),
                ]);
                return;
            }
            ...
            $stepDurationMs = (int) round((microtime(true) - $stepStartTime) * 1000);
        ...
        if ($stepIndex >= $maxSteps && $currentId) {
            $run->update([
                'status' => 'failed',
                'error' => "Execution reached maximum allowed steps limit ({$maxSteps}). Terminated to prevent runaway execution.",
                'completed_at' => now(),
            ]);
            return;
        }

        $totalDurationMs = $run->started_at ? (int) $run->started_at->diffInMilliseconds(now()) : 0;
        $run->update([
            'status' => 'completed',
            'completed_at' => now(),
    ...
    private function previewNode(string $type, array $data, Contact $contact, array $context): array
    {
        ...
        return match ($type) {
            ...
            'trigger_voice_agent', 'create_call', 'trigger_ai_call' => $ok('Would trigger AI voice call via Voice Agent #'.($data['voice_agent_id'] ?? '?').' to '.$contact->phone_e164),
    ...
    private function executeNode(array $node, AutomationRun $run, array $context): array
    {
        ...
        try {
            return match ($type) {
                ...
                'ai_followup', 'context_followup' => $this->executeAiFollowup($data, $run, $context),
                'human_handoff' => $this->executeHumanHandoff($data, $run, $context),
                'update_lead_score' => $this->executeUpdateLeadScore($data, $run, $context),
                'add_to_campaign' => $this->executeAddToCampaign($data, $run),
                // ── VOICE AGENTS ──────────────────────────────────────────────
                'trigger_voice_agent', 'create_call', 'trigger_ai_call' => $this->executeTriggerVoiceAgent($data, $run, $context),
                // ── SEND ──────────────────────────────────────────────────────
                ...
                'assign_agent', 'assign_user' => $this->executeAssignAgent($data, $run),
                'create_task', 'create_crm_task', 'followup_task' => $this->executeCreateTask($data, $run, $context),
                'move_pipeline_stage', 'update_stage' => $this->executeMovePipelineStage($data, $run, $context),
                'trigger_ai_agent' => $this->executeAiReply($data, $run, $context),
                // ── ENGAGE ────────────────────────────────────────────────────
    ...
    private function executeTriggerVoiceAgent(array $data, AutomationRun $run, array $context): array
    {
        $contact = Contact::find($run->contact_id);
        if (! $contact || ! $contact->phone_e164) {
            return ['status' => 'skipped', 'message' => 'Contact has no valid phone number.'];
        }

        $agentId = $data['voice_agent_id'] ?? null;
        $agent = $agentId ? \App\Modules\Voice\Models\VoiceAgent::find($agentId) : null;
        if (! $agent) {
            return ['status' => 'error', 'message' => 'Voice Agent not found.'];
        }

        $call = \App\Modules\Voice\Models\VoiceCall::create([
        ...
        try {
            $driverManager = app(\App\Modules\Voice\Services\VoiceDriverManager::class);
            $driver = $driverManager->driverForAgent($agent);
            $result = $driver->initiateOutboundCall($agent, $call, $contact->phone_e164);
            $call->update([
            ...
            return ['status' => 'ok', 'message' => "Voice call initiated via {$agent->name} to {$contact->phone_e164}."];
        } catch (\Throwable $e) {
            $call->update(['status' => 'failed', 'error_json' => ['error' => $e->getMessage()]]);

            return ['status' => 'error', 'message' => "Voice call failed: {$e->getMessage()}"];
        }
    }

    private function executeAiFollowup(array $data, AutomationRun $run, array $context): array
    {
        $contact = Contact::find($run->contact_id);
        if (! $contact) {
        ...
        $smartFollowUpService = app(\App\Modules\Automation\Services\SmartFollowUpService::class);
        if ($smartFollowUpService->shouldHaltFollowup($run)) {
            $run->update(['status' => 'cancelled', 'error' => 'Follow-up safely halted: customer replied, converted, or opted out.']);

            return ['status' => 'skipped', 'message' => 'Follow-up safely halted.'];
        }

        $workspaceId = (int) $run->automation->workspace_id;
        $body = $smartFollowUpService->generateContextAwareFollowup($contact, $workspaceId, $data['objective'] ?? null);
        $channel = $this->pickChannel($data);

        return $this->dispatchMessage($run, $channel, 'text', $body, null);
    }

    private function executeHumanHandoff(array $data, AutomationRun $run, array $context): array
    {
        $contact = Contact::find($run->contact_id);
        if (! $contact) {
            return ['status' => 'skipped', 'message' => 'Contact not found.'];
        ...
        $conversation = Conversation::where('contact_id', $contact->id)->latest('last_message_at')->first();
        if (! $conversation) {
            return ['status' => 'skipped', 'message' => 'No active conversation found for contact.'];
        }
        ...
        $handoffService->executeHandoff(
            ...
            $data['reason'] ?? 'Automated workflow handed over to human'
        );

        return ['status' => 'ok', 'message' => 'Conversation handed over to human agent.'];
    }

    private function executeUpdateLeadScore(array $data, AutomationRun $run, array $context): array
    {
        $contact = Contact::find($run->contact_id);
        if (! $contact) {
            return ['status' => 'skipped', 'message' => 'Contact not found.'];
        ...
        $scoringService = app(\App\Modules\Shared\Services\OmnichannelLeadScoringService::class);
        $scoringService->updateScore($contact, [
            'intent' => $data['intent'] ?? null,
            'custom_delta' => $data['delta'] ?? 10,
        ]);

        return ['status' => 'ok', 'message' => "Updated lead score for {$contact->full_name} to {$contact->lead_score} ({$contact->lead_score_band})."];
    }
    ...
    private function executeSendEmail(array $data, AutomationRun $run, array $context): array
    {
        ...
        \App\Modules\Shared\Models\ContactTimelineEvent::create([
            ...
            'metadata_json' => ['subject' => $subject, 'recipient' => $contact->email],
            'occurred_at' => now(),
        ]);
    ...
    private function executeAiReply(array $data, AutomationRun $run, array $context): array
    {
        ...
        if ($userMessage !== '' && app(\App\Modules\Inbox\Services\HumanHandoffService::class)->isHandoffRequested($userMessage)) {
            $conversation = Conversation::where('workspace_id', $workspaceId)->where('contact_id', $contact->id)->latest('id')->first();
            if ($conversation) {
                app(\App\Modules\Inbox\Services\HumanHandoffService::class)->executeHandoff($conversation, null, 'Customer requested human in automation workflow');
            }

            return ['status' => 'ok', 'message' => 'Customer requested live human agent. Handoff executed and AI stopped.'];
        }
    ...
    {
        // Contact name shorthand: {{contact.name}} / {{contact.full_name}} -> full name
        $template = str_replace(['{{contact.name}}', '{{contact.full_name}}'], (string) $contact->full_name, $template);
    ...
    private function executeWebhook(array $data, AutomationRun $run, array $context): array
    {
        ...
        $respBody = $response->json() ?? ['body' => substr($response->body(), 0, 500)];

        return [
            'status' => 'ok',
            'message' => "Webhook {$method} {$url} → {$response->status()}",
            'output' => ['status' => $response->status()],
            'provider_payload' => array_merge($payload, ['context' => $context]),
            'provider_response' => ['status' => $response->status(), 'data' => $respBody],
    ...
    public function evaluateCondition(array $data, ?Contact $contact, array $context): bool
    {
        ...
        // Response / Inbound message detection
        if (in_array($field, ['contact.has_replied', 'has_replied', 'conversation.has_replied'], true)) {
            $hasInbound = ($contact && $contact->exists)
                ? Message::where('contact_id', $contact->id)->where('direction', 'in')->exists()
                : false;

            return in_array($operator, ['not_equals', 'false', '0'], true) ? ! $hasInbound : $hasInbound;
        }

        if (in_array($field, ['no_reply', 'contact.no_reply'], true)) {
            $hasInbound = ($contact && $contact->exists)
                ? Message::where('contact_id', $contact->id)->where('direction', 'in')->exists()
                : false;
    ...
    private function executeCreateTask(array $data, AutomationRun $run, array $context): array
    {
        $contact = Contact::find($run->contact_id);
        if (! $contact) {
            return ['status' => 'skipped', 'message' => 'Contact not found.'];
        ...
        $workspaceId = $run->automation->workspace_id;
        $title = $this->renderTokens((string) ($data['title'] ?? 'Follow up with '.$contact->full_name), $contact, $context);
        $description = $this->renderTokens((string) ($data['description'] ?? 'Automated task created from workflow.'), $contact, $context);
        $priority = $data['priority'] ?? 'urgent';
        $assignedUserId = ! empty($data['assigned_user_id']) ? (int) $data['assigned_user_id'] : ($contact->assigned_user_id ?? User::where('workspace_id', $workspaceId)->where('status', 'active')->value('id'));

        $dueAt = now()->addMinutes(15);
        if (! empty($data['due_in_minutes'])) {
            $dueAt = now()->addMinutes((int) $data['due_in_minutes']);
        } elseif (! empty($data['due_in_hours'])) {
            $dueAt = now()->addHours((int) $data['due_in_hours']);
        }

        $task = \App\Models\Crm\CrmTask::create([
            'workspace_id' => $workspaceId,
            'contact_id' => $contact->id,
            'lead_id' => $contact->lead_id,
            'deal_id' => $contact->deals()->latest('id')->value('id'),
            'assigned_user_id' => $assignedUserId,
            'created_by_id' => $assignedUserId ?? $workspaceId,
            'title' => $title,
        ...
        \App\Modules\Shared\Models\ContactTimelineEvent::create([
            ...
            'description' => 'Task scheduled for '.($task->assignedUser?->name ?: 'Salesperson'),
            'metadata_json' => ['task_id' => $task->id, 'priority' => $task->priority],
            'occurred_at' => now(),
        ]);

        return ['status' => 'ok', 'message' => "Task '{$task->title}' created."];
    }
    ...
    private function executeMovePipelineStage(array $data, AutomationRun $run, array $context): array
    {
        $contact = Contact::find($run->contact_id);
        if (! $contact) {
            return ['status' => 'skipped', 'message' => 'Contact not found.'];
        ...
        $workspaceId = (int) $run->automation->workspace_id;
        $stageId = (int) ($data['stage_id'] ?? ($data['pipeline_stage_id'] ?? 0));
        $pipelineId = ! empty($data['pipeline_id']) ? (int) $data['pipeline_id'] : (int) $contact->pipeline_id;

        if (! $stageId) {
            return ['status' => 'error', 'message' => 'No target pipeline stage specified.'];
        }

        $stage = \App\Models\Crm\CrmPipelineStage::where('workspace_id', $workspaceId)->find($stageId);
        if (! $stage) {
            return ['status' => 'error', 'message' => "Pipeline stage {$stageId} not found in workspace."];
        }
        ...
        // Sync associated CRM deal if present
        $deal = \App\Models\Crm\CrmDeal::where('workspace_id', $workspaceId)
            ->where('contact_id', $contact->id)
            ->latest('id')
            ->first();
        ...
        return [
            'status' => 'ok',
            'message' => "Moved contact to stage '{$stage->name}'.",
            'output' => ['stage_id' => $stageId, 'stage_name' => $stage->name],
        ];
    }
}
