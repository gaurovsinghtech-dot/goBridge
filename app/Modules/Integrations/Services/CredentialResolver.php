...
class CredentialResolver
{
    ...
    public function twilio(): ?SmsCredentials
    {
        ...
        if ($config && $config->enabled) {
            ...
            if (! empty($creds['account_sid']) && ! empty($creds['auth_token'])) {
                return new SmsCredentials([
                    ...
                    'api_key_sid' => $creds['api_key_sid'] ?? null,
                    'api_key_secret' => $creds['api_key_secret'] ?? null,
                    'messaging_service_sid' => $creds['messaging_service_sid'] ?? null,
                    'voice_app_sid' => $creds['voice_app_sid'] ?? null,
                    'webhook_base_url' => $creds['webhook_base_url'] ?? null,
                ]);
            }
        }

        return $this->sms('twilio');
    }
    ...
    public function llm(string $provider): ?LlmCredentials
    {
        ...
        $default = $this->resolve('llm_'.$provider.'_default', LlmCredentials::class);
        if ($default) {
            return $default;
        }
        ...
        if ($aiConfig && $aiConfig->enabled) {
            $creds = $aiConfig->credentials ?? [];
            $key = $creds[$provider.'_api_key'] ?? '';
            if (! empty($key)) {
                return new LlmCredentials([
                    'api_key' => $key,
                    'model' => $creds[$provider.'_model'] ?? null,
                    'organization_id' => $creds['openai_organization_id'] ?? null,
                ]);
            }
        }
    ...
    public function sms(string $provider): ?SmsCredentials
    {
        ...
        if ($provider === 'twilio') {
            ...
            if ($config && $config->enabled) {
                $creds = $config->credentials ?? [];
                if (! empty($creds['account_sid']) && ! empty($creds['auth_token'])) {
                    return new SmsCredentials($creds);
                }
            }
        }
