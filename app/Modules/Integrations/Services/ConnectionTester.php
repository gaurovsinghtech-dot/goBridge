...
class ConnectionTester
{
    public function test(IntegrationConfig $config): array
    {
        try {
            $result = match (true) {
                $config->provider === 'twilio' => $this->testTwilio($config),
                $config->provider === 'meta_app' => $this->testMeta($config),
                $config->provider === 'ai_providers' => $this->testAiProviders($config),
                str_starts_with($config->provider, 'crm_') => $this->testCrm($config),
    ...
    public function testTwilio(IntegrationConfig $config): array
    {
        ...
        if ($resp->successful()) {
            ...
            return ['ok' => true, 'message' => "Connected to Twilio: {$name} (Status: {$status})"];
        }

        return ['ok' => false, 'message' => $resp->json()['message'] ?? 'Twilio authentication failed. Check Account SID & Auth Token.'];
    }

    public function testAiProviders(IntegrationConfig $config): array
    {
        $creds = $config->credentials ?? [];
        $defaultProvider = $creds['default_provider'] ?? 'openai';

        if ($defaultProvider === 'openai') {
            $apiKey = $creds['openai_api_key'] ?? '';
            if (empty($apiKey)) {
                return ['ok' => false, 'message' => 'OpenAI API key is required for default provider.'];
            }
            $model = $creds['openai_model'] ?? 'gpt-4o-mini';
            $resp = HttpFacade::timeout(15)->withToken($apiKey)
            ...
            return $resp->successful()
                ? ['ok' => true, 'message' => "OpenAI connection successful (Model: {$model})."]
                : ['ok' => false, 'message' => $resp->json()['error']['message'] ?? 'OpenAI connection error.'];
        }

        if ($defaultProvider === 'gemini') {
            $apiKey = $creds['gemini_api_key'] ?? '';
            if (empty($apiKey)) {
                return ['ok' => false, 'message' => 'Google Gemini API key is required for default provider.'];
            }
            $resp = HttpFacade::timeout(15)->get("https://generativelanguage.googleapis.com/v1beta/models?key={$apiKey}");

            return $resp->successful()
                ? ['ok' => true, 'message' => 'Google Gemini connection successful.']
                : ['ok' => false, 'message' => $resp->json()['error']['message'] ?? 'Gemini connection error.'];
        }

        if ($defaultProvider === 'anthropic') {
            $apiKey = $creds['anthropic_api_key'] ?? '';
            if (empty($apiKey)) {
                return ['ok' => false, 'message' => 'Anthropic Claude API key is required for default provider.'];
            }
            $model = $creds['anthropic_model'] ?? 'claude-3-haiku-20240307';
            ...
            return $resp->successful()
                ? ['ok' => true, 'message' => "Anthropic Claude connection successful (Model: {$model})."]
                : ['ok' => false, 'message' => $resp->json()['error']['message'] ?? 'Anthropic connection error.'];
        }

        return ['ok' => false, 'message' => "Unknown default AI provider: {$defaultProvider}"];
    }
    ...
    private function testMeta(IntegrationConfig $config): array
    {
        ...
        if (! empty($token)) {
            ...
            if ($resp->successful() && isset($resp->json()['id'])) {
                return ['ok' => true, 'message' => 'Connected to Meta WhatsApp Business API. System User ID: '.$resp->json()['id']];
            }
        }

        if (! empty($appId) && ! empty($appSecret)) {
            $resp = HttpFacade::timeout(10)->get('https://graph.facebook.com/oauth/access_token', [
                'client_id' => $appId,
                'client_secret' => $appSecret,
                'grant_type' => 'client_credentials',
            ]);
            if ($resp->successful() && isset($resp->json()['access_token'])) {
                return ['ok' => true, 'message' => 'Meta App credentials verified successfully.'];
            }
        }

        if (empty($appId) || empty($appSecret)) {
            return ['ok' => false, 'message' => 'Meta App ID and App Secret are required.'];
        }

        return ['ok' => false, 'message' => 'Could not verify Meta App credentials. Please check App ID and App Secret.'];
    }

    public function testCrm(IntegrationConfig $config): array
    {
        $crmManager = app(\App\Services\Crm\Connectors\CrmManager::class);
        $providerSlug = str_starts_with($config->provider, 'crm_') ? substr($config->provider, 4) : $config->provider;

        $creds = $config->credentials ?? [];
        $res = $crmManager->test($providerSlug, $creds);

        return [
            'ok' => $res['ok'] ?? false,
            'message' => $res['message'] ?? 'CRM connection test completed.',
            'checks' => $res['checks'] ?? [],
        ];
    }
    ...
    private function testStorage(IntegrationConfig $config): array
    {
        ...
        if ($config->provider === 'storage_s3') {
            $service = app(\App\Services\Storage\StorageService::class);
            return $service->testConnection($creds);
        }
