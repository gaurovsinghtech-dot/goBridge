...
class StorageManager
{
    ...
    private function buildDiskConfig(string $provider, array $creds): array
    {
        return match ($provider) {
            'storage_s3' => [
                'driver'                  => 's3',
                'key'                     => $creds['key']    ?? env('AWS_ACCESS_KEY_ID'),
                'secret'                  => $creds['secret'] ?? env('AWS_SECRET_ACCESS_KEY'),
                'region'                  => $creds['region'] ?? env('AWS_DEFAULT_REGION', 'us-east-1'),
                'bucket'                  => $creds['bucket'] ?? env('AWS_BUCKET'),
    ...
    {
        if (empty($diskConfig) || app()->environment('testing')) {
            return;
        }
