    ...
    public function sendRaw(
        ...
        try {
            if (app()->environment('testing')) {
                Mail::html($bodyHtml, function ($message) use ($to, $subject, $smtp, $extraHeaders, $fromEmail, $fromName, $replyTo) {
                    $message->to($to)
                        ->subject($subject)
                        ->from($fromEmail ?: ($smtp->from_email ?? 'noreply@growbridge.io'), $fromName ?: ($smtp->from_name ?? 'Growbridge'));
