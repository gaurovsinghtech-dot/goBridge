...
class AnalyticsService
{
    ...
    public function campaignDeliveryOverTime(int $campaignId): array
    {
        ...
        $sentExpr = $isSqlite ? "strftime('%Y-%m-%d %H:00', sent_at) as hour, COUNT(*) as total" : "DATE_FORMAT(sent_at, '%Y-%m-%d %H:00') as hour, COUNT(*) as total";
        $delExpr = $isSqlite ? "strftime('%Y-%m-%d %H:00', delivered_at) as hour, COUNT(*) as total" : "DATE_FORMAT(delivered_at, '%Y-%m-%d %H:00') as hour, COUNT(*) as total";
        $readExpr = $isSqlite ? "strftime('%Y-%m-%d %H:00', read_at) as hour, COUNT(*) as total" : "DATE_FORMAT(read_at, '%Y-%m-%d %H:00') as hour, COUNT(*) as total";
    ...
    public function aiKpis(int $wsId, Carbon $from, Carbon $to): array
    {
        $row = AiRun::query()
            ...
            ->selectRaw('COUNT(*) as total_runs,
                         ...
                         SUM(CASE WHEN ai_runs.status = "failed" THEN 1 ELSE 0 END) as failed_runs')
            ->first();
    ...
    {
        $isSqlite = \Illuminate\Support\Facades\DB::connection()->getDriverName() === 'sqlite';
        $weekExpr = $isSqlite ? "strftime('%Y-%W', created_at) as week, COUNT(*) as clients" : "DATE_FORMAT(created_at, '%Y-%u') as week, COUNT(*) as clients";

        $rows = Client::selectRaw($weekExpr)
