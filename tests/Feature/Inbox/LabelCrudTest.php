...
class LabelCrudTest extends TestCase
{
    ...
    public function test_cross_workspace_attach_forbidden(): void
    {
        ...
        $workspaceB = $ctxB['workspace'];

        $labelB = InboxLabel::create(['workspace_id' => $workspaceB->id, 'name' => 'B-Label', 'color' => '#0f0']);
        $contactA = Contact::factory()->create(['workspace_id' => $this->ctx['workspace']->id]);
        $accountA = ChannelAccount::create([
            'workspace_id' => $this->ctx['workspace']->id, 'channel' => 'whatsapp', 'provider' => 'meta',
            'display_name' => 'WA', 'status' => 'active',
        ]);
        $convA = Conversation::create([
            'workspace_id' => $this->ctx['workspace']->id,
            'channel_account_id' => $accountA->id,
            'contact_id' => $contactA->id,
            'status' => 'open',
        ]);

        $response = $this->actingAs($userA)->postJson(route('client.inbox.labels.attach', $convA), [
            'label_id' => $labelB->id,
        ]);
