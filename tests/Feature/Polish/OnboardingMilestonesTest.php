...
class OnboardingMilestonesTest extends TestCase
{
    ...
    public function test_connect_first_channel_auto_detected_after_channel_account_created(): void
    {
        ...
        $step = collect($progress['steps'])->firstWhere('key', 'whatsapp')
            ?? collect($progress['steps'])->firstWhere('key', 'connect_first_channel');
        ...
        $step2 = collect($progress2['steps'])->firstWhere('key', 'whatsapp')
            ?? collect($progress2['steps'])->firstWhere('key', 'connect_first_channel');
    ...
    {
        ['user' => $user, 'workspace' => $workspace] = $this->createWorkspaceContext();
        $workspace->update(['service_type' => 'whatsapp_voice']);

        $service = $this->makeService();
        $progress = $service->getProgress($user);

        $step = collect($progress['steps'])->firstWhere('key', 'phone')
            ?? collect($progress['steps'])->firstWhere('key', 'import_first_contacts');
        ...
        $progress2 = $service->getProgress($user->refresh());
        $step2 = collect($progress2['steps'])->firstWhere('key', 'phone')
            ?? collect($progress2['steps'])->firstWhere('key', 'import_first_contacts');
    ...
    public function test_train_first_chatbot_auto_detected(): void
    {
        ...
        $step = collect($progress['steps'])->firstWhere('key', 'ai_agent')
            ?? collect($progress['steps'])->firstWhere('key', 'train_first_chatbot');
        $this->assertFalse($step['completed']);
        ...
        $progress2 = $service->getProgress($user->refresh());
        $step2 = collect($progress2['steps'])->firstWhere('key', 'ai_agent')
            ?? collect($progress2['steps'])->firstWhere('key', 'train_first_chatbot');
        $this->assertTrue($step2['completed']);
    }
