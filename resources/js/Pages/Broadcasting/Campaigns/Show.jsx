...
export default function CampaignShow({ campaign, sample = [], reportUrl }) {
    ...
    const metrics = [
        { key: 'total', label: t('campaign.metric_total', 'Total Audience'), value: totals.total ?? 0, color: 'bg-neutral-500' },
        { key: 'sent', label: t('campaign.metric_sent', 'Sent'), value: totals.sent ?? 0, color: 'bg-blue-500' },
        { key: 'delivered', label: t('campaign.metric_delivered', 'Delivered'), value: totals.delivered ?? 0, color: 'bg-green-500' },
        { key: 'read', label: t('campaign.metric_read', 'Read'), value: totals.read ?? 0, color: 'bg-purple-500' },
        { key: 'replied', label: t('campaign.metric_replied', 'Replies'), value: totals.replied ?? campaign.replied_count ?? 0, color: 'bg-emerald-500' },
        { key: 'failed', label: t('campaign.metric_failed', 'Failed'), value: totals.failed ?? 0, color: 'bg-red-500' },
    ];
                    ...
                    <div className="ml-auto flex flex-wrap gap-2">
                        ...
                        {!['queued', 'sending'].includes(campaign.status) && (
                            <button
                                ...
                                className="flex items-center gap-1.5 rounded-lg border border-red-300 dark:border-red-800 text-red-600 dark:text-red-400 px-3 py-2 text-sm font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition"
                            >
