...
import { Key, Plus, Trash2, Copy, Check, ShieldCheck, ExternalLink } from 'lucide-react';
    ...
    return (
        <ClientLayout title={t('api.tokens_title')}>
            ...
            <div className="space-y-6 max-w-5xl">
                <ApiConnectionsNav current="tokens" />

                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-xl bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400 flex items-center justify-center">
                            <Key className="h-5 w-5" />
                        </div>
                        <div>
                            <h1 className="text-xl font-bold text-neutral-900 dark:text-white">{t('api.tokens_title', 'API Keys & Access Tokens')}</h1>
                            <p className="text-xs text-neutral-500 dark:text-neutral-400">{t('api.tokens_subtitle', 'Authenticate REST API requests to automate contacts, campaigns, messages & webhooks.')}</p>
                        </div>
                    </div>
                    <button
                        onClick={() => setShowCreate(true)}
                        className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-700 shadow-sm transition-all"
                    >
                        <Plus className="h-4 w-4" /> {t('api.create_token', 'Create API Key')}
                    </button>
                </div>
