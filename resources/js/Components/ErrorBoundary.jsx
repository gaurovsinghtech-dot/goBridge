        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-neutral-50 dark:bg-neutral-950 p-6">
                <div className="max-w-md w-full rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 p-8 text-center shadow-sm">
                    ...
                    {this.state.error && (
                        <details className="mt-4 text-left text-xs text-neutral-600 dark:text-neutral-400 bg-neutral-100 dark:bg-neutral-800 p-3 rounded-lg overflow-auto max-h-48">
                            <summary className="cursor-pointer font-medium text-neutral-800 dark:text-neutral-200">Error Details</summary>
                            <p className="mt-2 font-mono text-red-600 dark:text-red-400">{String(this.state.error?.message || this.state.error)}</p>
                            {this.state.error?.stack && <pre className="mt-1 font-mono text-[10px] whitespace-pre-wrap">{this.state.error.stack}</pre>}
                        </details>
                    )}
